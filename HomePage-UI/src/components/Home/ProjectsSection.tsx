          ...
          >
            <div className="p-6">
              ...
              <div className="flex flex-wrap gap-2 mb-4">
                {proj.tags.map(tag => (
                  <span key={tag} className="px-2 py-1 text-xs bg-emerald-400/10 text-emerald-400 rounded-full">
                    {tag}
                  </span>
                ))}
              </div>
              <div className="flex items-center gap-4 mt-auto">
                <a href={proj.github} target="_blank" rel="noopener noreferrer" className="flex items-center gap-1 text-sm font-medium hover:text-emerald-400 transition-colors">
                  <GithubIcon className="w-4 h-4" /> GitHub
                </a>
                {proj.live && (
                  <a href={proj.live} target="_blank" rel="noopener noreferrer" className="flex items-center gap-1 text-sm font-medium hover:text-emerald-400 transition-colors">
